...
class Category
{
    ...
    public static function create($newData)
    {
        ...
        if (existeCategoriaId($dataArray, $newData['categoryId'])) {
        ...
        if (count($arrayErrores) > 0) { // Si el array de errores es mayor que 0, entonces  creo un array asociativo que mostrará la respuesta
            ...
            $newId = nextId($dataArray); // Llamo a la función nextId para asignarle un id correcto al nuevo usuario
...
function existeCategoriaId($dataArray, $categoryId)
{
...
// Función para garantizar que no se duplique el ID
function existeIdExcluyendo($dataArray, $idObjeto, $id)
{
    foreach ($dataArray as $data) {
        if ($data['categoryId'] === $idObjeto && $data['id'] !== $id) {
